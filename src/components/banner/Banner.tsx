    ...
    return (
        <div className="w-full bg-custom-green relative pb-10" style={{height: "550px"}}>
            ...
            <div className="flex px-4 absolute bottom-0 left-1/2 transform -translate-x-1/2 w-full sm:w-3/4 justify-center">
                <button className={`${selectedButton === 1 ? 'bg-custom-gray ' : 'bg-custom-green border-custom-green'} font-semibold text-xs py-4 px-2 border border-b-0 rounded-t-lg flex items-center md:text-base sm:w-5/12`} onClick={() => handleButtonClick(1)}><span className='mr-4'><MessageIcon/></span> {button1}</button>
                <button className={`${selectedButton === 2 ? 'bg-custom-apricot ' : 'bg-custom-green border-custom-green'} font-semibold text-xs py-4 px-2 border border-b-0 rounded-t-lg flex items-center md:text-base sm:w-5/12`} onClick={() => handleButtonClick(2)}><span className='mr-4'><BookIcon /></span> {button2}</button>
                {/* #fff0d9 */}
